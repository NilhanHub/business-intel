"""Run the Phase 1 dry-run LLM classification review harness."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="UK/IE D365 Phase 1 LLM classification review dry run")
    parser.add_argument("--live-llm", action="store_true", help="Future Phase 2 only; refused in Phase 1")
    parser.add_argument("--max-candidates", type=int, default=20, help="Future live review candidate cap")
    parser.add_argument("--model", default=None, help="Future live review model override")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.live_llm:
        print(json.dumps(live_llm_refusal(), indent=2, sort_keys=True))
        return 2
